...
function OpenIDConnectStrategy({
  ...
  if (sessionKey && storage) {
    throw new TypeError('must give only one of sessionKey and storage');
  }
  ...
  this._store = storage || new SessionStore(sessionKey || client);
...
OpenIDConnectStrategy.prototype.authenticate = function authenticate(req, options) {
  (async () => {
    ...
    if (Object.keys(reqParams).length === 0) {
      ...
      const session = pick(params, 'nonce', 'state', 'max_age', 'response_type');
      ...
      await this._store.save(req, session, params);
    ...
    const session = await this._store.load(req, options);
